...
AccelHelper accelHelper(10, 5000, 40000);
...
//void leftTurn45() 	{ delay(1000); bot->turn(-45,  &rightTurn45); }
//void rightTurn45() 	{ delay(1000); bot->turn( 45,  &leftTurn180); }
//void leftTurn180() 	{ delay(1000); bot->turn(-180, &rightTurn180);}
//void rightTurn180()	{ delay(1000); bot->turn( 180, &pause);       }
//void pause()        { delay(5000); }
...
bool stopWhenPickedUp() {
	if (accelHelper.acceleratedZ()) {
		blink3();
		if (running) {
			bot->stop();
			delay(1000);
...
bool reactToHittingSomething() {
	if (accelHelper.acceleratedX()) {
		blink3();
		delay(500);
		blink3();
		goBack();
...
void loop() {
	...
	(bot->isManeuvering()) || reactToHittingSomething(); // || stopWhenPickedUp();
